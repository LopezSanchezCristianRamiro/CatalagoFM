...
export default function ProductoImagenesModal({
  ...
  return (
    <>
              ...
              {fotos.length > 0 ? (
                ...
                >
                  {fotos.map((foto, index) => (
                    <View
                      key={foto.idFotoProducto ?? index}
                      style={{ width: 280 }}
                    >
                      <Pressable onPress={() => setImagenCompleta(foto.urlFoto)}>
                        <Image
      ...
      <Modal visible={imagenCompleta !== null} transparent animationType="fade">
        <View className="flex-1 bg-black/90 items-center justify-center px-4">
          <Pressable
            onPress={() => setImagenCompleta(null)}
            className="absolute top-12 right-6 w-12 h-12 rounded-full bg-white/20 items-center justify-center z-10"
          >
          ...
          <ThemedText className="text-white/80 mt-4">
