...
type CartAnimCtx = {
  ...
  triggerFly: (fromX: number, fromY: number, cardW: number, cardH: number, imageUrl?: string) => void;
  flyState: FlyState;
  resetFly: () => void;
};
...
export function CartAnimationProvider({ children }: { children: React.ReactNode }) {
  ...
  const [flyState, setFlyState] = useState<FlyState>({
    visible: false, fromX: 0, fromY: 0, toX: 0, toY: 0, cardW: 0, cardH: 0,
  });

  const triggerFly = (fromX: number, fromY: number, cardW: number, cardH: number, imageUrl?: string) => {
    cartRef.current?.measureInWindow((x, y, w, h) => {
      setFlyState({
        visible: true,
        fromX, fromY, cardW, cardH, imageUrl,
        toX: x + w / 2,
        toY: y + h / 2,
      });
    });
  ...
  return (
    <CartAnimationContext.Provider value={{ cartRef, triggerFly, flyState, resetFly }}>
      {children}
