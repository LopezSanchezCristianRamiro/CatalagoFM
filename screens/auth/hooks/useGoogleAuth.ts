...
const redirectUri = AuthSession.makeRedirectUri({ scheme: "com.catalogofm" });
...
export function useGoogleAuth() {
  ...
  // Configuración del flujo OAuth (igual que en la prueba)
  ...
  // Envía idToken al backend para verificar existencia
  const verifyTokenOnBackend = async (idToken: string) => {
    ...
    try {
      ...
      if (backendResponse.status === "success") {
        // Usuario existe: usar el token de Sanctum devuelto
  ...
  // Inicia todo el flujo (al presionar botón)
  const startGoogleSignIn = useCallback(async () => {
    ...
    try {
      await promptAsync({ windowFeatures: { width: 600, height: 700 } });
    } catch (error: any) {
      Toast.show({
        type: "error",
        text1: "Error",
  ...
  // Cuando el usuario ingresa teléfono en el modal
  ...
  return {
    ...
    handlePhoneSubmit,
    closePhoneModal,
    requestReady: !!request, // para deshabilitar botón mientras no está listo
  };
}
